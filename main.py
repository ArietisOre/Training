...
def pluralise(number, singular, plural):
    """Returns the correct singular or plural word 
        based on the number provided. Used to display 
...
if __name__ == "__main__":
    digits = choose_four_digit_number()
        ...
        if bulls == 4:
            if attempts == 1:
                ...
                if ask_to_play_again():
                    ...
                    break
        if bulls < 4:
            print(f"{bulls} {pluralise(bulls, 'bull', 'bulls')}, {cows} {pluralise(cows, 'cow', 'cows')}")